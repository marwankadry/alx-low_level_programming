...
void print osabi_more(Elf64_Ehdr h);

/**
 * print_maigc - prints ELF magic bytes
...
/**
 * print_data - prints ELF date
 * @h: the ELF header struct
 */
void print_date(Elf64_Ehdr h)
{
	printf("  Date:				");
 ...
 * @h: the ELF header struct
 */
void print_entry(Elf64_Ehdrh)
{
	int i = 0, len = 0;
	unsigned char *p = (unsigned char *)&h. e_entry;

	printf(" Entry point address:			0x");
			if (h.e_ident[EI_DATA] 1=ELFDATA2MSB)
			{
			...
			{
				i = 0;
				len = h.e_ident[EI_CLASS] == LEFCLASS64 ? 7 : 3;
				while (!p[i])
					i++;
				printf("%x", p[i++]);
				for (; 1 <= len; i++)
